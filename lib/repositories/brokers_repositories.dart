...
class BrokersRepo{
    ...
    try {
      final data = jsonEncode(
          Get5PaisaDataModel(
            ...
            body: Body(encryKey:encryKey ,requestToken: RequestToken,userId: userId)
          )
      );

      print('get5PaisaAccessToken-data---------$data');

      final response = await ApiHandler.postWithoutBaseURL(url: get5PaisaAccessTokenUrl, body: data);
  ...
  get5PaisaHoldingsData({
    ...
    try {
      final data = jsonEncode(
          Get5PaisaHoldingsDataModel(
              head: HoldingHead(key: userKey),
              body: HoldingBody(clientCode: ClientCode)
          )
      );
      print('get5PaisaAccessToken-data---------$data');
      final response = await ApiHandler.post3(url: get5PaisaHoldingUrl, body: data,accessToken: AccessToken);
      print('api--order--response------${response.statusCode}');
      print(response.statusCode);
      return response;
    } on BadRequestException {
